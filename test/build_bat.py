# -*- coding: utf-8 -*-
"""生成 UTF-8(无BOM) + CRLF 的启动脚本。

cmd 批处理要求 CRLF 换行（LF 会导致解析器把行切碎）。
第2行 chcp 65001 让中文在 GBK 代码页的系统上也正常显示。
"""
...
out = Path(r"D:/NoVoice") / "启动工具.bat"
data = CONTENT.replace("\r\n", "\n").replace("\n", "\r\n").encode("utf-8")
assert not data.startswith(b"\xef\xbb\xbf")
out.write_bytes(data)
print("OK", out, len(data), "bytes, CRLF+UTF-8(no BOM)")
